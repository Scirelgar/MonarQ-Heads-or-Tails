...
class MenuBar:
    ...
    def _handle_submenu_click(self, pos):
        ...
        # First check if click is on a nested submenu (like Device options)
        for i, item in enumerate(submenu_items):
            if "submenu" in item:
                ...
                # Check if click is in nested submenu area
                nested_rect = pygame.Rect(
                    nested_submenu_x, item_y, max_width, nested_height
                )
                if nested_rect.collidepoint(pos):
                    # Handle click on nested submenu item
                    for j, nested_item in enumerate(nested_items):
                        nested_item_y = item_y + j * item_height
                        nested_item_rect = pygame.Rect(
                            nested_submenu_x, nested_item_y, max_width, item_height
                        )

                        if (
                            nested_item_rect.collidepoint(pos)
                            and "action" in nested_item
                        ):
    ...
    def _is_mouse_over_submenu_area(self, pos):
        ...
        # Check nested submenu areas (like Device submenu)
        for i, item in enumerate(submenu_items):
            if "submenu" in item and item["text"] == "Device":
                item_y = submenu_y + i * item_height
                item_rect = pygame.Rect(submenu_x, item_y, max_width, item_height)
                nested_submenu_x = submenu_x + max_width
                nested_items = item.get("submenu", [])
                ...
                # Check if mouse is over nested submenu
                nested_rect = pygame.Rect(
                    nested_submenu_x, item_y, max_width, nested_height
                )
                if nested_rect.collidepoint(pos):
                ...
                # Check if mouse is in corridor between parent and nested submenu
                if self._should_show_nested_submenu(
                    item_rect, nested_submenu_x, item_y, pos
                ):
                    return True
    ...
    def _should_show_nested_submenu(self, parent_rect, nested_x, nested_y, mouse_pos):
        ...
        # Show if hovering over the area where nested submenu would appear
        # (this prevents flickering when moving mouse to nested submenu)
        nested_area = pygame.Rect(
            nested_x, nested_y, 200, 100
        )  # Approximate nested submenu area
        if nested_area.collidepoint(mouse_pos):
            return True

        # Show if mouse is in the "corridor" between parent and nested submenu
        # This creates a safe zone for mouse movement
        corridor_rect = pygame.Rect(
            parent_rect.right,
            min(parent_rect.top, nested_y),
            nested_x - parent_rect.right + 10,  # Small overlap
            max(parent_rect.height, 100),
        )
    ...
    def _render_submenu(self, screen):
        ...
        for i, item in enumerate(submenu_items):
            ...
            # Render nested submenu if this item has one and mouse is nearby
            if (
                "submenu" in item
                and enabled
                and item["text"] == "Device"
                and self._should_show_nested_submenu(
                    item_rect, submenu_x + max_width, item_y, mouse_pos
                )
            ):
                self._render_nested_submenu(screen, item, submenu_x + max_width, item_y)

    def _render_nested_submenu(self, screen, parent_item, submenu_x, submenu_y):
        """
        Render a nested submenu (like Device options).
